...
for i in data:
    if int(sys.argv[2]) in i['sighted_at']:
        ...
        abbr = tmpt[1]
        if abbr not in check:
            check.append(abbr)
            if abbr in state_abbr and i['population'] != '':
                indice = state_abbr.index(abbr)
                state_name = state[indice]
                if state_name in coor_state:
                    lons.extend([coor_state[state_name][1]]*int(i['population']))
                    lats.extend([coor_state[state_name][0]]*int(i['population']))
...
cbar.set_label('Population',size=18)
#plt.clim([0,100])


# translucent blue scatter plot of epicenters above histogram:
x,y = m(lons, lats)
plt.title('Population in US 2005')
